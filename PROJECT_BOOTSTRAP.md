...
Reusable prompt for scaffolding a new Expo app from a reference architecture. Copy the block below into a new agent chat and fill in the placeholders.

---

```
Bootstrap a new Expo React Native app using my reference repository as the architectural template.

### Inputs
- **Reference repo (template):** <REFERENCE_REPO_URL>
...
### Goal
Create the initial project with the same **tooling, folder conventions, and design-token workflow** as the reference repo — adapted to this project's Figma file and feature scope. When **New project GitHub repo** is provided, deliver an initial commit there; otherwise scaffold locally and report the working directory path.
...
Do not copy demo-only screens, APIs, or domain features unless I ask. Copy the **platform setup**, **token pipeline** (when Figma is provided), **theming approach**, **lint/test/CI**, and **component/file conventions**.

---

### Project scaffold
Always bootstrap from the official Expo default template for SDK 56 — **do not** hand-roll `package.json` or copy the reference repo wholesale.
...
2. **Remove unnecessary default template files** before layering reference architecture:
   - Demo routes and screens (e.g. tab explore/demo flows, sample modals) — replace with the minimal placeholder shell defined below
   - Template-only components (`components/ui/*`, parallax/demo helpers, stock `ThemedText` / `ThemedView` when superseded by project primitives)
   ...
   - Restructure to match reference conventions when needed (e.g. move `app/` → `src/app/`, wire `@/` path aliases in `tsconfig.json`)
3. **Install additional packages** required by this prompt and the reference repo:
   ...
   - Enable only optional dependencies listed in **Optional capabilities**
4. **Copy and adapt config from the reference repo** (`biome.json`, `eslint.config.mjs`, `metro.config.js`, `jest.config.js`, CI workflow, token/icon scripts, Argent rules) — do not copy demo app code or domain features.

Then continue with design tokens, icons, architecture, and the minimal initial app.
...
- **expo-router** — file-based routing, typed routes; enable React Compiler if the reference does
...
- **Biome** — lint + format (`biome.json`):
  - `a11y` preset
  - `nursery/noReactNativeRawText` with the project's text primitive component(s) in `skip` (reference may use something like `ThemedText` — use whatever this app defines)
  - `style/useFilenamingConvention` with path overrides for component vs route naming (see conventions below)
  - Exclude generated tokens, generated assets, and native folders from lint
- **ESLint** — React Native accessibility only (`eslint.config.mjs`):
...
- **Jest** — `jest-expo`, `@testing-library/react-native`, `@/` path aliases
- **GitHub Actions CI** — `bun install --frozen-lockfile` → `bun run lint` → `bun run test`
- **TypeScript** — strict mode, `@/` path aliases
- **State / storage** — Zustand + react-native-mmkv; shared MMKV adapter in `src/lib/mmkv.ts`; persisted stores in `src/stores/` (e.g. theme and language preferences)

### Optional stack (enable only when listed in Inputs)
- **Storybook (on-device)** — `.rnstorybook/`, gated by `EXPO_PUBLIC_STORYBOOK_ENABLED`; colocate component stories; keep design-token stories under `src/stories/design-tokens/` when used
- **i18n** — `i18next`, `react-i18next`, `expo-localization`
- **GraphQL** — Apollo Client v4 with codegen (`graphql:generate`); operations in `src/services/graphql/operations/*.graphql`, generated output in `src/services/graphql/generated/`, singleton client in `src/services/graphql/client.ts`; env endpoint `EXPO_PUBLIC_GRAPHQL_URL`; **cache persist (MMKV via `apollo3-cache-persist`) and startup prefetch enabled by default** via `prefetchQueries.ts`; WebSocket subscriptions **optional** (`EXPO_PUBLIC_GRAPHQL_SUBSCRIPTIONS_ENABLED=true`, optional `EXPO_PUBLIC_GRAPHQL_WS_URL`); no demo queries unless I ask
- **Icons** — when **Figma icons section** is provided, follow the icon pipeline below (SVG → `react-native-nano-icons` font); otherwise use whatever the Figma file and product require, or omit until a link is provided
...
3. **Adapt the generator per project** (do not assume the reference Figma structure):
...
The reference generator (`generate-design-tokens.mjs`) is a **starting point**, not a fixed spec. Trim or extend it to match this project's Figma file and goals.
...
4. Wire the reference icon font pipeline:
   ...
   - `Icon` wrapper in `src/components/Icon/` with typed `name`, `size`, and `color` / `colorToken` props (match reference)
   - `IconFontLoader` in root layout when fonts are required
5. Exclude `assets/icons/**` from Biome/ESLint where the reference does; add a design-token Storybook grid under `src/stories/design-tokens/Icons.stories.tsx` when Storybook is enabled.
6. After adding or changing SVGs, regenerate the font/glyphmap per the reference (`bun run` script or plugin build step) and verify `Icon` renders a sample set at multiple sizes and color tokens.

If the icons link is omitted, scaffold `assets/icons/app-icons/` empty (or with a minimal placeholder) only when I asked for the icon font pipeline in **Optional capabilities**.
...
### App architecture (conventions, not a fixed file list)
    ...
    src/
      ...
      theme/
        ...
        typography.ts              # className helpers for tokens (name as needed)
        tokens/raw/
        tokens/generated/
...
**File naming (enforce with Biome `useFilenamingConvention` overrides):**
...
**Styling conventions:**
- Route all user-facing text through a small set of token-aware text primitive(s) using generated typography tokens
...
### Minimal initial app
Replace reference demo content with a thin shell appropriate to this product. Unless I specify otherwise:
- A simple root layout and one or two placeholder screens
- Settings or equivalent screen only if I asked for theme/language toggles
- No sample API, gallery, or domain-specific flows unless I list them in Inputs
...
**Required:**
- `start`, `ios`, `android`
- `lint` (`biome check . && eslint .`), `lint:fix`, `lint:a11y` (if split out in reference)
- `test`, `test:watch`
...
### Git deliverable
1. **Project root:** if **New project GitHub repo** is provided, clone it locally (or use an existing empty repo checkout). Otherwise, create a new local directory (e.g. next to the reference clone) and `git init` with `main` as the default branch.
2. **Scaffold** per **Project scaffold** above (`bunx create-expo-app@latest … --template default@sdk-56`, remove template cruft, install additional packages, adapt reference config).
...
5. **Device smoke test (before commit/push, when Argent is available):** confirm Argent is installed (`command -v argent` or `mcp__argent__*` tools present). If yes, run the app on **both iOS and Android devices** (simulator, emulator, or connected hardware) and verify there are no issues before pushing:
   - Copy Argent config from the reference repo when present (`.cursor/rules/argent.md`, `.cursor/mcp.json` or `.mcp.json`, related skills/agents).
   - Boot or use a running iOS simulator and Android emulator (`list-devices` → prefer already-booted targets).
   - Start Metro if needed, then launch the app on each platform (`launch-app` / Expo dev client workflow per reference).
...
### Constraints
- Start from `bunx create-expo-app@latest … --template default@sdk-56` — do not skip the official template or clone the reference app source as the project base
...
**Match reference repo literally**
```
Mirror optional dependencies and folder layout from <REFERENCE_REPO_URL> unless they conflict with my Inputs above.
```
...
```
Before the initial push, if Argent is installed, boot iOS simulator and Android emulator, launch the app on both, and confirm no crashes/redboxes and the placeholder shell renders correctly. Report devices used and any issues fixed.
```
