...
- **Optional capabilities** (include only what the user lists): Storybook, i18n, GraphQL subscriptions, custom icon font pipeline (enable automatically when **Figma icons section** is provided)
...
3. **Install additional packages** per **Required stack** and **Optional capabilities** (see `templates/README.md` **Installing dependencies**):
   ...
   - **Live install logs (required for agents):**
     - One Shell tool call per package — run the bare install command only (e.g. `bun add uniwind`, `bun add -d storybook`, `bunx expo install expo-localization`). Do **not** prefix with `echo`, pipe to `tail`, or redirect stdout/stderr.
     - Keep each install in the **foreground** with a generous `block_until_ms` so Bun output streams while resolving/downloading. Never background installs (`block_until_ms: 0`) or batch multiple packages in one command.
     - Use the shell step **description** (e.g. `Install storybook`) for progress context in the UI — not echoed text in the command.
     - Wait for the command to finish; read full stdout/stderr (peer-dep warnings, errors) before starting the next package. Non-zero exit = hard stop.
     - If a single package hangs >2 minutes on `Resolving dependencies`, kill that process and retry **only that package** once before escalating.
...
Templates ship a thin shell: Home + Settings tabs, theme/language toggles on Settings, root layout with providers. Unless the user specifies otherwise:

- Keep or simplify that shell — do not add sample APIs, galleries, or domain flows unless listed in Inputs
