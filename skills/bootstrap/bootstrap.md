...
2. **Remove cruft** — demo routes, stock helpers, non-Bun lockfiles, and any web leftovers (`*.web.*`, `web` script/config, favicon / tutorial-web assets). Current default scaffold usually already uses `src/app/` and `@/` — only move `app/` → `src/app/` or add path aliases if missing. Apps are **iOS/Android only**. Strip any leftover agent-assembly comments (intake toggles, “swap for drawer”, “uncomment when…”) — keep those instructions in skill/README only, not in app source.
