...
- Uniwind: CSS `src/global.css`; Metro `withUniwindConfig` outermost; `bunx uniwind generate-artifacts …` before Phase C. For native color props (`Icon`, `ActivityIndicator`, etc.) use `useTokenColor` — **never** dynamic `` `accent-${token}` `` (build-time class detection fails)
